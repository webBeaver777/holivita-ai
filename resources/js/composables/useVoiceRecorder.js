...
export function useVoiceRecorder() {
     ...
     */
    async function transcribe(audioData, userId, sessionId = null, provider = null) {
        ...
        try {
            ...
            formData.append('audio', audioData.blob, `recording.${audioData.extension}`)
            ...
            const submitResponse = await axios.post('/api/voice/async/transcribe', formData, {
            ...
            if (!submitResponse.data.success) {
                error.value =
                    submitResponse.data.error || 'Не удалось отправить аудио на распознавание.'
                return null
            }
            ...
            const result = await pollTranscriptionStatus(transcriptionId, userId)
            return result
        } catch (err) {
            ...
            try {
                ...
                if (data.completed) {
                    if (data.status === 'completed') {
                        ...
                        error.value = data.error || 'Ошибка распознавания речи.'
                    }
                    return null
                }
    ...
    function handleTranscriptionError(err) {
        if (err.response?.status === 503) {
            error.value = 'Сервис распознавания речи недоступен. Проверьте настройки провайдера.'
        } else if (err.response?.status === 422) {
            const message = err.response?.data?.errors?.audio?.[0] || err.response?.data?.message
