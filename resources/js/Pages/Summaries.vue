    ...
    <div class="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 p-4">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-6 mb-6">
                <div class="flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <input v-model="searchUserId" type="number" min="1" placeholder="User ID"
                            @keyup.enter="search"
                        ...
                        <button v-if="searchUserId" @click="resetSearch"
            ...
            <div v-else class="space-y-4">
                <div v-for="summary in summaries" :key="summary.id" @click="openDetails(summary)"
                ...
                <div class="flex items-center justify-between bg-white rounded-xl shadow-md p-4">
                    <button @click="prevPage" :disabled="!hasPrevPage || isLoading"
                    ...
                    <button @click="nextPage" :disabled="!hasNextPage || isLoading"
                        class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        Вперёд
                    </button>
                </div>
        ...
        <Transition enter-active-class="transition-opacity duration-200" enter-from-class="opacity-0"
            ...
            <div v-if="selectedSummary"
                ...
                <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
                    <!-- Modal Header -->
                    <div
                        class="bg-gradient-to-r from-purple-500 to-blue-500 text-white p-4 flex items-center justify-between">
                        <div>
                    ...
                    <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                        <div class="space-y-4">
                            ...
                            <!-- Health Goals -->
                            <div v-if="selectedSummary.summary?.health_goals?.length"
                                class="bg-green-50 rounded-xl p-4">
                                <h3 class="text-sm font-semibold text-green-700 mb-2">Цели здоровья</h3>
                                <ul class="list-disc list-inside text-gray-700 space-y-1">
                                    <li v-for="goal in selectedSummary.summary.health_goals" :key="goal">
                                        {{ goal }}</li>
                                </ul>
                            </div>

                            <!-- Current Health Issues -->
                            <div v-if="selectedSummary.summary?.current_health_issues?.length"
                                class="bg-orange-50 rounded-xl p-4">
                                <h3 class="text-sm font-semibold text-orange-700 mb-2">Текущие проблемы</h3>
                                <ul class="list-disc list-inside text-gray-700 space-y-1">
                                    <li v-for="issue in selectedSummary.summary.current_health_issues"
                                        :key="issue">{{ issue }}</li>
                                </ul>
                            </div>
                            ...
                            <div v-if="selectedSummary.summary?.lifestyle" class="bg-blue-50 rounded-xl p-4">
                                ...
                                <div class="grid grid-cols-2 gap-3 text-sm">
                                    <div v-if="selectedSummary.summary.lifestyle.sleep">
                                        ...
                                        <span
                                            class="text-gray-700 ml-1">{{ selectedSummary . summary . lifestyle . sleep }}</span>
                                    </div>
                                    <div v-if="selectedSummary.summary.lifestyle.nutrition">
                                        <span class="text-gray-500">Питание:</span>
                                        <span
                                            class="text-gray-700 ml-1">{{ selectedSummary . summary . lifestyle . nutrition }}</span>
                                    </div>
                                    <div v-if="selectedSummary.summary.lifestyle.activity">
                                        <span class="text-gray-500">Активность:</span>
                                        <span
                                            class="text-gray-700 ml-1">{{ selectedSummary . summary . lifestyle . activity }}</span>
                                    </div>
                                    <div v-if="selectedSummary.summary.lifestyle.stress">
                                        <span class="text-gray-500">Стресс:</span>
                                        <span
                                            class="text-gray-700 ml-1">{{ selectedSummary . summary . lifestyle . stress }}</span>
                                    </div>
                                </div>
                            </div>

                            <!-- Recommendations -->
                            <div v-if="selectedSummary.summary?.recommendations_focus?.length"
                                class="bg-indigo-50 rounded-xl p-4">
                                <h3 class="text-sm font-semibold text-indigo-700 mb-2">Фокус рекомендаций</h3>
                                <ul class="list-disc list-inside text-gray-700 space-y-1">
                                    <li v-for="rec in selectedSummary.summary.recommendations_focus"
                                        :key="rec">{{ rec }}</li>
                                </ul>
                            </div>

                            <!-- Raw JSON (if structure is different) -->
                            <div v-if="!selectedSummary.summary?.summary_text && !selectedSummary.summary?.health_goals"
