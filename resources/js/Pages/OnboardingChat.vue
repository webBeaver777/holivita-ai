<script setup>
    ...
    const canSend = computed(() => {
        return userInput.value.trim().length > 0 && !isLoading.value && !isCompleted.value;
    });
    ...
    async function startOnboarding() {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Добавляем приветствие ассистента
                messages.value.push({
                    role: 'assistant',
    ...
    async function completeOnboarding() {
        try {
            ...
            if (response.data.success) {
                ...
                messages.value.push({
                    role: 'assistant',
                    content: '🎉 Отлично! Я узнал тебя получше. Теперь смогу давать более персональные рекомендации. Добро пожаловать в Holivita!'
                });
            }
        } catch (err) {
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div v-if="!userId" class="bg-white rounded-2xl shadow-xl p-8 mt-8">
                ...
                <div class="max-w-sm mx-auto">
                    ...
                    <input v-model="userIdInput" type="number" min="1" placeholder="Например: 1"
                    ...
                    <!-- Кнопка -->
                    <button @click="validateUserId" :disabled="!userIdInput || isValidating"
            ...
            <div v-else class="flex flex-col h-[calc(100vh-2rem)]">
                <!-- Header -->
                <header class="bg-white rounded-t-2xl shadow-lg p-4 border-b border-gray-100 flex-shrink-0">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <button v-if="isCompleted" @click="resetChat"
                ...
                <!-- Messages Area -->
                <div ref="messagesContainer" class="flex-1 overflow-y-auto bg-white/70 backdrop-blur-sm p-4 space-y-4">
                    <!-- Сообщения -->
                    ...
                    <div v-if="error" class="flex justify-center">
                        <div class="bg-red-50 text-red-600 rounded-xl px-4 py-2 text-sm border border-red-200">
                            {{ error }}
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-b-2xl shadow-lg p-3 flex-shrink-0">
                    <div class="flex items-end space-x-3">
                        <textarea v-model="userInput" @keydown="handleKeydown" :disabled="isLoading || isCompleted"
                            :placeholder="isCompleted ? 'Знакомство завершено' : 'Напиши сообщение...'" rows="1"
